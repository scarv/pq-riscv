...
)
{
	...
	shake256(c + SYND_BYTES, 32, two_e, sizeof(two_e)); 

	memcpy(one_ec + 1, e, SYS_N/8);
	memcpy(one_ec + 1 + SYS_N/8, c, SYND_BYTES + 32);

	shake256(key, 32, one_ec, sizeof(one_ec));
...
)
{
	...
	shake256(conf, 32, two_e, sizeof(two_e)); 
	...
	shake256(key, 32, preimage, sizeof(preimage)); 
...
)
{
	...
	{
		rp = r;
		pqriscv_stream_aes256ctr(r, sizeof(r), nonce, seed);
